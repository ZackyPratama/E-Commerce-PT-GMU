            ...
            <div>
                ...
                @if($product->review_count > 0)
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <span class="text-gray-600">{{ number_format($product->average_rating, 1) }} ({{ $product->review_count }} reviews)</span>
                    </div>
                @endif
                ...
                <div class="mb-6">
                    @if($selectedVariant)
                        ...
                        <div class="flex items-center gap-3">
                            <span class="text-3xl font-bold text-gray-900">Rp {{ number_format($variant->price, 3) }}</span>
                            @if($variant->compare_price)
                                <span class="text-xl text-gray-500 line-through">Rp {{ number_format($variant->compare_price, 3) }}</span>
                    ...
                    @else
                        <div class="flex items-center gap-3">
                            <span class="text-3xl font-bold text-gray-900">Rp {{ number_format($product->price, 3) }}</span>
                            @if($product->compare_price)
                                <span class="text-xl text-gray-500 line-through">Rp {{ number_format($product->compare_price, 3) }}</span>
                                <span class="bg-red-100 text-red-800 px-2 py-1 rounded text-sm font-semibold">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($product->variants->where('is_active', true) as $variant)
                                <button wire:click="selectVariant({{ $variant->id }})"
                                    ...
                                    <p class="text-sm text-gray-600">Rp {{ number_format($variant->price, 2) }}</p>
